...
func (m *sqlSink) writeToDB(ctx api.StreamContext, sqlStr *string) error {
	...
	fmt.Println("begin write to db.....................", *sqlStr)
	r, err := m.db.Exec(*sqlStr)
	if err != nil {
		fmt.Println("%s: %s", errorx.IOErr, err.Error())
		return fmt.Errorf("%s: errorx%s", errorx.IOErr, err.Error())
	}
