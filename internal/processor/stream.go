...
func (p *StreamProcessor) RecoverLookupTable() error {
	...
	for _, k := range keys {
		...
		if ok, _ := p.db.Get(k, &v); ok {
			fmt.Println("the p.db.Get is...", v)
			if err := json.Unmarshal([]byte(v), vs); err == nil && vs.StreamType == ast.TypeTable {
				fmt.Println("in this stage, lookup tables are recovered., we are not managed it..............................................")
...
func (p *StreamProcessor) execSave(stmt *ast.StreamStmt, statement string, replace bool) error {
	...
	fmt.Println("StreamProcessor execSave......................... ", statement)
	...
	fmt.Println("create a new stream ok....................", string(stmt.Name), string(s), replace)
