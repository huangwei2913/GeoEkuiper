...
func (rr *RuleRegistry) Store(key string, value *rule.RuleState) {
	...
	rr.Unlock()
	globalcache.Set(key, value, cache.NoExpiration)
}
...
//store the rule state uid and its ts.toplohy
func createRuleState(r *api.Rule) (*rule.RuleState, error) {
	rs, err := rule.NewRuleState(r)
	if err != nil {
		return rs, err
	}
	fmt.Println("...................createRuleState.......", r.Id)
	registry.Store(r.Id, rs)
	return rs, nil
}

func createRuleStateForPeer(r *api.Rule) (*rule.RuleState, error) {
	rs, err := rule.NewRuleStateForPeer(r)
	if err != nil {
		return rs, err
	}
	fmt.Println("...................createRuleState.......", r.Id)
...
func recoverRuleForPeer(r *api.Rule) string {
	// Validate the topo
	rs, err := createRuleStateForPeer(r)
	if err != nil { // when recovering rules, assume the rules are valid, so always add it to the registry
		conf.Log.Errorf("Create rule topo error: %v", err)
		r.Triggered = false
		registry.Store(r.Id, rs)
...
func createRuleForPeer(r *api.Rule) string {
...
//obtain the stream souces from rule sql
...
func startRuleForPeer(name string) error {
	rs, ok := registry.Load(name)
	if !ok {
		return fmt.Errorf("Rule %s is not found in registry, please check if it is created", name)
	} else {
...
func getRuleStatus(name string) (string, error) {
	if rs, ok := registry.Load(name); ok {
		fmt.Println("out runing here getRuleStatus..............")
		...
		fmt.Println("does runing into this erroo....................")
